...
export default function ArchivedConfirm({ open, handleClose, currentStatus }) {
    ...
    const archiveProjectInWorkspace = useProjectStore(state => state.archiveProjectInWorkspace)

    const handleArchived = async () => {
        try {
            await archiveProjectInWorkspace(selectedProject._id)
            handleClose()
        } catch (err) {
            console.log(err)
        }
        ...
        >
            ...
            <DialogTitle id="status-dialog-title" component="div" sx={{ textAlign: 'center', pt: 2 }}>
                <Stack direction="row" alignItems="center" justifyContent="center" spacing={1.5}>
                    ...
                    <ArrowForwardIcon sx={{ color: '#4c1c94', fontSize: '1.2rem' }} />
                    <Typography variant="h6" sx={{ color: '#4c1c94', fontWeight: 800 }}>
                        ARCHIVED
                    </Typography>
                </Stack>
            </DialogTitle>
            ...
            <DialogContent sx={{ textAlign: 'center', py: 1 }}>
                <DialogContentText id="status-dialog-description" component="div">
                    ...
                    <Typography variant="caption" sx={{ color: '#6D28D9' }}>
                        <strong>{currentStatus}</strong>에서 <strong>ARCHIVED</strong>로 상태가 업데이트됩니다.
                    </Typography>
                </DialogContentText>
            </DialogContent>
